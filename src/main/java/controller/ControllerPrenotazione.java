 ...
 */
public class ControllerPrenotazione<T extends Turista,R extends IRicerca> {
     ...
     * @param r un derivato della ricerca
     */
    public ControllerPrenotazione(T t, R r,IDB database){
        if(t!=null && r!=null && database!=null){
            this.t = t;
            this.r = r;
    ...
    private Esperienza setExp(){
        List<Esperienza> lista = r.risultatoRicerca();
    ...
    private void conferma(IBuilderInterface iBuilderInterface,IGestoreDisponibilita iGestoreDisponibilita){
        ...
        if(s.equals("y")){
            gestorePrenotazioni.aggiungiPrenotazione(new ObjPrenotazione((BuilderPrenotazione) iBuilderInterface.getResult()));
    ...
    public void modificaStatoPrenotazione(){
        gestorePrenotazioni.showAll();
        List<IObjPrenotazione> recupero = gestorePrenotazioni.recuperaEsperienzeAssociate();
        cli.message("indice della prenotazione da modificare");
        int indice = -1;
        IObjPrenotazione iObjPrenotazione = checkIndex(recupero,indice);
        ...
        String s = cli.cmd();
        if(s.equals("y")){
            IObjPrenotazione iObjPrenotazione1 =  new ObjPrenotazione((BuilderPrenotazione) iBuilderInterface.buildRiserva(false).getResult());
            gestorePrenotazioni.annullaPrenotazione(indice);
    ...
    private IObjPrenotazione checkIndex(List<IObjPrenotazione> recupero,int indice) {
        indice = parseInt();
        try{
